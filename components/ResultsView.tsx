...
const ResultsView: React.FC<ResultsViewProps> = ({ schemes, onBack, isDirectory = false }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-12 md:py-24 animate-reveal relative">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-[1000px] h-[400px] md:h-[600px] bg-premium-primary/5 rounded-full blur-[100px] md:blur-[150px] -z-10"></div>
      ...
      <div className="max-w-4xl mx-auto mb-10 md:mb-12">
        ...
        <div className="flex items-center gap-3 md:gap-4 mb-4">
          ...
          <h2 className={`${isDirectory ? 'text-2xl md:text-4xl' : 'text-3xl md:text-5xl'} font-black text-white tracking-tighter leading-tight`}>
            {isDirectory ? t('results.directory_title') : t('results.title')}
          </h2>
        </div>

        <p className="max-w-3xl text-sm md:text-lg text-white/60 leading-relaxed font-medium">
          {isDirectory
      ...
      {schemes.length > 0 ? (
        ...
        <div className="glass-card rounded-[2rem] md:rounded-[4rem] py-16 md:py-24 px-6 md:px-10 text-center">
          <h3 className="text-2xl md:text-4xl font-black text-white/20 mb-6 md:mb-8 tracking-tighter">{t('results.no_matches')}</h3>
          <p className="text-sm md:text-base text-white/40 max-w-md mx-auto mb-8 md:mb-10 font-medium leading-relaxed px-4">
